...
2) a WassersteinGan with gradient penality trained on 5000 **ground truth** MNIST images.
...
For M > 120, our model outperforms the GAN trained on ground truth data in terms of the distortion based normalized MSE metric. In general, if the range of the GAN captures natural MNIST images properly during training, then any GAN-based reconstruction is ensured to be a natural MNIST digit.
...
A fair comparison is to compare with the AmbientGAN [3] trained on compressed data generated by varying Gaussian measurement matrices. However, up to now, we could not sucessfully train an AmbientGAN with compressed data from Gaussian measurement matrices (see Fig. 13 for the generated MNIST images of this AmbientGAN). This is in line with [3], which observes severe performance degredation, when the measurement matrix is constructed from 1D (Gaussian) projections (see Figure 5(b) and "Section 8 - Failure Case" in [3]). It should be noted that these measurement matrices are exactly those, which are used for the comparison between our model sVaeCom with the GAN trained on ground truth data in Fig. 7-9.

---

Figure 10: Reconstruction performance comparison between
1) sVaeCom trained on a training set of 5000 **compressed** MNIST images with 
2) an AmbientGan trained on 5000 **compressed** MNIST images.

The compressed MNIST images were generated by varying masking measurement matrices, which randomly masked out 50% (M=392) and 75% (M = 191) pixels.

<img src="ambientgancomparison.jpg" width=60% height=60%>

---

Figure 11: Exemplary reconstructed images from MNIST for M = 392 with

- the AmbientGAN trained on compressed data and
...
To evaluate the proper training of the GANs, we plotted generated MNIST images with the GAN trained on ground truth data (Fig. 12), with the AmbientGAN trained on compressed data with Gaussian measurement matrices (Fig. 13), and with the AmbientGAN trained on compressed data with masked pixels (Fig. 14). Moreover, we added a detailed description of the GAN models and the compressive sensing reconstruction at the very end.


---


Figure 12: Generated MNIST images with the GAN trained on ground truth data.

<img src="gtGanGenerated.jpg" width=100% height=100%>

---

Figure 13: Generated MNIST images with the AmbientGAN trained on compressed data with Gaussian measurement matrices. The training failed.

<img src="ambientGanGeneratedGaussian.jpg" width=100% height=100%>

---

Figure 14: Generated MNIST images with the AmbientGAN trained on compressed data with measurement matrices, which randomly mask out half of the pixels. The perceptual quality roughly matches the one in [3], which generated MNIST images in the same setting in Fig. 11 (fourth column, second row).
