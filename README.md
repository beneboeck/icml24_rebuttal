...
Therefore, although our model outperforms the GAN for M = 120 in terms of the nMSE, perceptually both performs arguably similar for M = 200 (see Fig. 8) and for M = 120, the GAN is perceptually better (see Fig. 9). While our model recovers the details of the compressed MNIST image, the GAN emphasizes the perceptual quality of the image.
However, the conclusions from this comparison is limited since our model learns from compressed data of dimension M, whereas the GAN can access ground truth data.

---

Figure 8: Exemplary reconstructed images from MNIST for M = 200 of the sVaeCom trained on compressed data and the GAN trained on ground truth data.

<img src="gtGan_sVaeComM200.jpg" width=100% height=100%>

---

Figure 9: Exemplary reconstructed images from MNIST for M = 120 of the sVaeCom trained on compressed data and the GAN trained on ground truth data.
